...
- `src/ui/` – React components; views are chosen by the legacy hash links (`route.ts`), and the organizer views by the `code` parameter of `radio.php` (`-2` left in forest, `0` start registration); `?comp=<uuid>&scroll` is the scrolling page of `followallscroll.php`. `App.test.tsx` renders the app against a fake `fetch` serving the fixtures. Colours are CSS variables in `styles.css`; the theme follows `prefers-color-scheme` unless the toggle in the top bar or footer (`ThemeToggle.tsx`) or `?theme=auto|classic|dark` picks one, remembered in `localStorage`. Dark-theme rules live under `:root[data-theme='dark']` and must not change the classic look.
